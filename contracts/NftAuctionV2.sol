...
// import "@openzeppelin/contracts/src/v0.8/shared/interfaces/AggregatorV3Interface.sol";

contract NftAuctionV2 is Initializable {//is Initializable, UUPSUpgradeable
    struct Auction {
        //卖家
        address seller;
        //拍卖持续时间
    ...
    // AggregatorV3Interface priceFeed; // 如果需要价格预言机，可以添加此字段
    ...
    //     return price;
    // }       
    // function setProceETHFeed(address _priceFeed) public {
    ...
    function createAuction(uint256 _duration,uint256 _startPrice,address _nftAddress,uint256 _nfpId)  public {
        ...
        // approve
        // IERC721(_nftContract).approve(address(this), _nfpId); // 授权合约可以操作NFT  
        ...
        auctions[nextAuctionId] = Auction ({
            ...
            // tokenAddress: _tokenAddress // 如果需要代币地址，可以添加此字段
        });
        nextAuctionId++;
    }
    ...
    //     //如果有最高出价者，转账给卖家
    //     if (auction.highestBidder != address(0)) {
    //         payable(address(this)).transfer(address(this).balance); // 将合约中的余额转给卖家
    //         //转移NFT给最高出价者
